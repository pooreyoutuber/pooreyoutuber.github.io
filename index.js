...
// 🛑 AUTOMATICALLY LOADED PROXY LIST FROM UPLOADED FILE 🛑
// These are the HTTP proxies (IP:PORT) from proxyscrape_premium_http_proxies (1).txt
// Since the proxies don't have usernames/passwords, we use them directly.
// The list is global, so we use one pool for all traffic.
const ALL_GLOBAL_PROXIES = [
    ...
    '104.207.60.58:3129', '209.50.166.110:3129', '209.50.170.93:3129', '216.26.254.100:3129',
    '209.50.164.168:3129', '104.207.57.162:3129', '65.111.15.170:3129', '209.50.170.126:3129',
    '209.50.188.66:3129', '65.111.6.214:3129', '104.207.44.84:3129', '104.207.40.98:3129',
    '65.111.24.172:3129', '216.26.254.110:3129', '45.3.42.225:3129', '45.3.55.246:3129',
    '65.111.15.15:3129', '65.111.29.210:3129', '216.26.229.214:3129', '45.3.32.13:3129',
    '45.3.53.142:3129', '154.213.160.98:3129', '65.111.1.33:3129', '216.26.237.142:3129',
    '104.207.36.219:3129', '45.3.39.66:3129', '45.3.44.176:3129', '104.207.60.243:3129',
    '104.207.52.73:3129', '65.111.5.122:3129', '216.26.253.178:3129', '104.207.34.14:3129',
    '154.213.166.61:3129', '209.50.171.188:3129', '65.111.7.44:3129', '65.111.4.124:3129',
    '216.26.228.120:3129', '216.26.232.191:3129', '65.111.30.12:3129', '45.3.45.87:3129',
    '104.207.40.63:3129', '104.207.37.61:3129', '216.26.253.33:3129', '104.207.36.147:3129',
    '45.3.32.221:3129', '154.213.161.220:3129', '65.111.8.148:3129', '104.207.43.160:3129',
    '65.111.8.236:3129', '154.213.164.107:3129', '209.50.191.236:3129', '65.111.7.120:3129',
    '209.50.179.65:3129', '65.111.13.25:3129', '104.207.47.183:3129', '65.111.26.13:3129',
    '216.26.250.229:3129', '104.207.62.125:3129', '209.50.190.245:3129', '216.26.233.180:3129',
    '209.50.172.51:3129', '45.3.51.2:3129', '209.50.166.230:3129', '104.207.45.3:3129',
    '104.207.42.200:3129', '45.3.41.231:3129', '104.207.49.44:3129', '45.3.51.234:3129',
    '45.3.36.2:3129', '209.50.186.85:3129', '104.207.34.99:3129', '209.50.163.251:3129',
    '216.26.231.232:3129', '209.50.178.133:3129', '45.3.45.19:3129', '65.111.12.11:3129',
    '65.111.12.100:3129', '216.26.242.199:3129', '65.111.31.65:3129', '154.213.163.95:3129',
    '45.3.50.117:3129', '65.111.22.188:3129', '104.207.37.202:3129', '154.213.160.201:3129',
    '209.50.160.27:3129', '65.111.28.111:3129', '104.207.33.105:3129', '216.26.241.226:3129',
    '104.207.46.76:3129', '216.26.240.92:3129', '216.26.251.165:3129', '45.3.47.110:3129'
];
// We'll treat all 15 target countries as having access to this global pool of proxies
const GLOBAL_COUNTRIES = ["US", "IN", "CA", "GB", "AU", "DE", "FR", "JP", "BR", "SG", "AE", "ES", "IT", "MX", "NL"];

// A list of modern User-Agents for realism
const USER_AGENTS = [
...
// --- Middleware and API Key Loading (Same as before) ---
...
async function sendGa4Hit(gaId, apiSecret, distribution, countryCode, realEvents) {
    ...
    if (ALL_GLOBAL_PROXIES.length > 0) {
        ...
        proxyUrl = `http://${ALL_GLOBAL_PROXIES[proxyIndex]}`; // Use http:// prefix
        agent = new HttpsProxyAgent(proxyUrl);
    } else {
        console.warn("WARNING: No proxies available. Traffic will use Render IP.");
    }
    ...
    events.push({
        ...
        params: {
            ...
            // We include geo_country, though the IP is the primary factor for GA Realtime
            user_properties: {
    ...
    try {
        ...
        if (response.status !== 204) {
         ...
         console.error(`Network Error (Proxy or Connection) for ${countryCode} using ${proxyUrl || 'Direct'}:`, error.message);
    }
}


// --- AI Endpoints (Same as before) ---
app.post('/api/ai-caption-generate', checkAi, async (req, res) => {
    ...
    const prompt = `Generate exactly ${count} Instagram Reels captions (in Hindi/Hinglish if topic is popular, otherwise English) for a video about "${description}". The style should be ${style}. Provide only the list of captions, each on a new line, without numbering or any other text.`;
...
app.post('/boost-mp', async (req, res) => {
    ...
    if (ALL_GLOBAL_PROXIES.length === 0) {
        console.warn("CRITICAL: PROXY LIST IS EMPTY. Traffic boosting will use Render IP and may not appear diversified.");
    }
    ...
    (async () => {
        ...
        console.log(`BOOST JOB COMPLETE: Successfully attempted ${successfulAttempts} views.`);
    })();
