...
// ===================================================================
// NEW TOOL 9: REMOTE VIRTUAL BROWSER (Gmail Session & Remote Control)
// ===================================================================
const archiver = require('archiver'); // npm install archiver (Render par auto-install ho jayega)

// Session folder jahan Gmail login save rahega
const SESSION_PATH = path.join(__dirname, 'gmail_session');
if (!fs.existsSync(SESSION_PATH)) fs.mkdirSync(SESSION_PATH);

let v_browser;
let v_page;

// A. Start/Toggle Browser Mode
app.get('/remote/start', async (req, res) => {
    const isDesktop = req.query.view === 'desktop';
    try {
        if (v_browser) await v_browser.close();

        v_browser = await puppeteer.launch({
            headless: "new",
            userDataDir: SESSION_PATH, // YAHAN COOKIES SAVE HOTI HAIN
            args: [
                '--no-sandbox', 
                '--disable-setuid-sandbox',
                '--disable-blink-features=AutomationControlled'
        ...
        v_page = await v_browser.newPage();
        
        if (isDesktop) {
            await v_page.setViewport({ width: 1280, height: 720 });
        } else {
            await v_page.setViewport({ width: 390, height: 844, isMobile: true, hasTouch: true });
        }

        // Default Google par le jao login ke liye
        await v_page.goto('https://accounts.google.com', { waitUntil: 'networkidle2' });
        res.json({ success: true, mode: isDesktop ? 'Desktop' : 'Mobile' });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});

// B. Screen Stream (Frontend isi ko refresh karega)
app.get('/remote/screen', async (req, res) => {
    if (v_page) {
        const screenshot = await v_page.screenshot();
        res.contentType('image/png').send(screenshot);
    } else {
        res.status(404).send("Browser Not Initialized");
    }
});

// C. URL Navigation
app.get('/remote/navigate', async (req, res) => {
    const url = req.query.url;
    if (v_page && url) {
        const target = url.startsWith('http') ? url : `https://${url}`;
        await v_page.goto(target, { waitUntil: 'networkidle2' });
        res.send("Navigated");
    }
});

// D. SESSION DOWNLOAD (Zip Creator)
app.get('/remote/download-session', (req, res) => {
    const zipName = 'gmail_session.zip';
