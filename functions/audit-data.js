...
exports.handler = async (event, context) => {
    ...
    let query = event.queryStringParameters.query; // query ko 'let' se define karein taaki use modify kar saken

    // --- ZAROORI CODE ADDED YAHAN ---
    // Safai: Input mein se unwanted symbols aur URL encoding ko theek karein.
    if (query) {
        // @ symbol hatao (agar user ne handle daala hai to)
        query = query.replace('@', ''); 
        // URL-encoded spaces (+) ko regular space mein badlo.
    ...
    try {
        ...
        // Ab 'query' variable cleaned ho chuka hai, isliye search theek se kaam karega.
        if (!query.startsWith('UC') || query.length !== 24) { 
            const searchRes = await youtube.search.list({
                q: query, // Cleaned query yahan use hoga
                type: 'channel',
                part: 'id',
                maxResults: 1,
            });
        ...
        // Fetch videos using playlistItems.list
        ...
        let videosWithStats = [];
        if (videoIds) {
            // Fetch video statistics and snippets in a separate call
